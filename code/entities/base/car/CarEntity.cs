...
public partial class CarEntity : Prop, IUse
{
	...
	protected void Tick()
	{
		if ( Driver is Character player && player.LifeState != LifeState.Alive )
		{
			RemoveDriver( player );
		}
	}
	...
	void SimulateDriver( Client client )
	{
		...
		{
			RemoveDriver( Driver as Character );
			return;
		}
	...
	private void RemoveDriver( Character player )
	{
		Driver = null;
	...
	{
		if ( user is Character player && timeSinceDriverLeft > 1.0f )
		{
			player.Parent = this;
	...
	protected override void OnPhysicsCollision( CollisionEventData eventData )
	{
		...
		if ( eventData.Entity is Character )
			return;
		...
		if ( speed > minImpactSpeed )
		{
			if ( eventData.Entity.IsValid() && eventData.Entity != this )
			{
				...
				if ( eventData.Entity.LifeState == LifeState.Dead && eventData.Entity is not Character )
				{
