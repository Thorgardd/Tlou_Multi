...
public class BaseTextEntry : TextEntry
{
	...
	protected BaseTextEntry(string name = "", Styles styles = null, List<Rule> rules = null)
	{
		Rules = new List<Rule>();
	...
	public bool HasRule()
	{
		return Rules.Count >= 1 ? true : false;
	}
	
	public bool CheckRule()
	{
		if ( Rules.Count < 1 ) return true;
		foreach ( var rule in Rules )
		{
			if ( !rule.CheckIsValid( this.Text ) ) {
